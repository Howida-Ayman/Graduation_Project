...
{
    /**
     * جلب المشرفين المتاحين (دكاترة ومعيدين)
     */
    ...
    public function availableSupervisors(HttpRequest $request)
    {
        ...
        // التأكد إن المستخدم هو Leader في فريق
        ...
        $query = User::whereIn('role_id', [2, 3]) // دكتور (2) أو معيد (3)
            ->where('is_active', true);
        
        // ✅ فلتر حسب النوع (doctor / ta)
        ...
        // استثناء المشرفين اللي بالفعل بيشرفوا على الفريق
        ...
        $supervisors = $query->get(['id', 'full_name', 'email', 'role_id', 'track_name']);
    ...
    /**
     * إرسال طلب إشراف (من الفريق للمشرف)
     */
public function requestSupervision(HttpRequest $request)
{
    $user = $request->user();
    
    // التأكد إن الطالب في فريق
    $membership = TeamMembership::where('student_user_id', $user->id)
        ->where('status', 'active')
        ->first();
    ...
    // ✅ 1. التأكد إن المستخدم هو الـ Leader
    if ($team->leader_user_id != $user->id) {
        return response()->json([
            'success' => false,
    ...
    // التأكد إن المستخدم دكتور أو معيد
    ...
    // ✅ 3. التأكد إن الفريق معندوش دكتور أو معيد من نفس النوع بالفعل
    ...
    // التأكد إن المشرف مش بيشرف على الفريق بالفعل
