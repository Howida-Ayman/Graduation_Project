    ...
    public function availableStudents(HttpRequest $request)
{
    ...
    $sentRequests = Request::where('from_user_id', $user->id)
        ->where('status', 'pending')
        ->whereHas('toUser.enrollments', function ($q) use ($academicYear) {
            $q->where('academic_year_id', $academicYear->id)
              ->where('status', 'active');
        })
        ->pluck('to_user_id')
    ...
    public function sendRequest(HttpRequest $request)
{
    ...
    $duplicatePending = Request::where('from_user_id', $user->id)
    ...
    try {
        ...
        if ($request->status == 'accepted') {
            if ($req->request_type == 'team_join') {
                ...
                $fromUserEnrollment = User::find($req->from_user_id)?->enrollments()
                    ->where('academic_year_id', $academicYear->id)
                    ->where('status', 'active')
                    ->exists();

                $toUserEnrollment = User::find($req->to_user_id)?->enrollments()
                    ->where('academic_year_id', $academicYear->id)
                    ->where('status', 'active')
                    ->exists();

                if (!$fromUserEnrollment || !$toUserEnrollment) {
                    DB::rollBack();
                    return response()->json([
                        'success' => false,
                        'message' => 'Both students must be active in the current academic year'
                    ], 400);
                }

                $fromHasTeam = TeamMembership::where('student_user_id', $req->from_user_id)
                    ->where('academic_year_id', $academicYear->id)
                    ->where('status', 'active')
                    ->exists();
                ...
                if ($fromHasTeam || $toHasTeam) {
                    DB::rollBack();
                    return response()->json([
                        'success' => false,
                        'message' => 'One of the students is already in a team'
                    ], 400);
                }
